...
class FeteLiteApp {
  ...
  showUpdateAvailableNotification() {
    ...
    updateModal.innerHTML = `
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p>${t('update.newVersionReady', { version: '1.0.10' })}</p>
          <p style="color: var(--color-text-secondary); font-size: var(--font-size-sm); margin-top: var(--space-3);">
            ${t('update.description') || 'Nowa wersja zawiera poprawki błędów i nowe funkcje.'}
          </p>
        </div>
        <div class="modal-footer">
          ...
          <button onclick="app.reloadForUpdate()" class="btn btn-primary">
            ${t('update.updateNow')}
          </button>
        </div>
      </div>
    ...
    // Dodaj obsługę zamykania na ESC i kliknięcie tła
    this.setupUpdateModalEvents(updateModal);
    
    this.showModal('update-modal');
  }

  // Konfiguracja event listenerów dla update modala
  setupUpdateModalEvents(modal) {
    ...
    // Zapobiegnij zamykaniu przy kliknięciu w treść modala
    modal.querySelector('.modal-content').addEventListener('click', (e) => {
      e.stopPropagation();
    });
  }

  // Przeładuj dla aktualizacji
  async reloadForUpdate() {
    try {
      // Pokaż spinner progress
      this.showUpdateProgress();
      
      // Wyłącz przyciski modala
      ...
      if ('serviceWorker' in navigator) {
        ...
        if (registration) {
          // Wymuś aktualizację cache przez Service Worker
          console.log('[App] Wymuszenie aktualizacji cache...');
          const messageChannel = new MessageChannel();
          
          // Promise dla odpowiedzi od Service Worker
          const cacheUpdatePromise = new Promise((resolve, reject) => {
            messageChannel.port1.onmessage = (event) => {
              const { type, error } = event.data;
              if (type === 'CACHE_UPDATED') {
                console.log('[App] Cache został zaktualizowany');
                resolve();
              } else if (type === 'CACHE_UPDATE_ERROR') {
                console.error('[App] Błąd aktualizacji cache:', error);
            ...
            setTimeout(() => {
              reject(new Error('Timeout podczas aktualizacji cache'));
            }, 30000);
          });
          
          // Wyślij wiadomość do Service Worker
          ...
          // Poczekaj na zaktualizowanie cache
          await cacheUpdatePromise;
          
          // Jeśli jest nowy Service Worker czekający, aktywuj go
          if (registration.waiting) {
            console.log('[App] Aktywacja nowego Service Worker...');
            registration.waiting.postMessage({ type: 'SKIP_WAITING' });
          }
          
          // Wymuś sprawdzenie aktualizacji
          console.log('[App] Sprawdzanie aktualizacji...');
          await registration.update();
          
          // Odczekaj chwilę dla synchronizacji
          await new Promise(resolve => setTimeout(resolve, 1000));
          
          // Przeładuj stronę
          console.log('[App] Przeładowanie aplikacji...');
          window.location.reload(true); // Wymuś przeładowanie z serwera
        } else {
          // Fallback - zwykłe przeładowanie
      ...
      console.error('[App] Błąd podczas aktualizacji:', error);
      
      // Pokaż error i przywróć modal
  ...
  showUpdateProgress() {
    // Usuń poprzedni progress jeśli istnieje
    const existing = document.getElementById('update-progress-overlay');
    if (existing) {
      existing.remove();
    }
    ...
    progressOverlay.innerHTML = `
      <div class="update-progress-content">
        ...
        <h3>${t('update.updating')}</h3>
        <p>${t('update.description') || 'Pobieranie najnowszej wersji aplikacji...'}</p>
        <div class="progress-steps">
          <div class="step active">📦 ${t('update.step1') || 'Pobieranie plików'}</div>
          <div class="step">🔄 ${t('update.step2') || 'Aktualizacja cache'}</div>
          <div class="step">✨ ${t('update.step3') || 'Finalizacja'}</div>
        </div>
      </div>
    `;
    ...
    // Animuj kroki postupnie
    setTimeout(() => {
      const steps = progressOverlay.querySelectorAll('.step');
  ...
  showUpdateError(errorMessage) {
    // Usuń progress
    const progress = document.getElementById('update-progress-overlay');
    if (progress) {
      progress.remove();
    }
    ...
    errorToast.className = 'update-error-toast';
